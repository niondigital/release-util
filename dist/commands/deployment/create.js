...
function getDeploymentBranches() {
    ...
    return process.env.DEPLOYMENT_BRANCHES.split(',').map((branch) => branch.trim());
}
export default async function createDeployment() {
    const deploymentBranches = getDeploymentBranches();
    ...
    const gitTagOutput = shell.exec("git tag -l --sort=-v:refname --format='%(refname:short)|%(creatordate)'", { silent: true });
    const releaseTagChoices = gitTagOutput.stdout
        .split('\n')
        .filter((tagString) => !!tagString)
        .map((tagString) => {
        const [tagName, creatorDate] = tagString.split('|');
        const creatorDateObject = new Date(creatorDate);
        return {
            value: tagName,
            short: tagName,
            name: `${tagName.padEnd(12, ' ')}   (${creatorDateObject.getDate()}.${creatorDateObject.getMonth() + 1}.${creatorDateObject.getFullYear()})`
        };
    });
    ...
    const statusResult = shell.exec('git status --porcelain');
    const hasLocalChanges = statusResult.stdout.split('\n').length > 1;
    if (hasLocalChanges) {
